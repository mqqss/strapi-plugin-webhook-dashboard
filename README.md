...
- Adds saved-config test triggers and execution history in the Settings page
...
## Local linked testing

You can test this plugin inside another local Strapi project without publishing it.
...
From your Strapi test project:

```bash
npm link strapi-plugin-webhook-dashboard
```
...
Then rebuild and start the Strapi test app:
...
When you change the plugin locally, run `npm run build` in this plugin folder again, then rebuild/restart the Strapi test app so the admin bundle picks up the latest files.
